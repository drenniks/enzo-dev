...
/  date:       January, 2022 - double stars
/
/  PURPOSE:
/
...
int grid::TestStarParticleInitializeGrid(float TestStarParticleStarMass_one,
           ...
           FLOAT TestStarParticleStarVelocity_two[],
					 FLOAT TestStarParticleStarPosition_two[],
			     ...
			     float PhotonTestInitialFractionH2II) 
{
  ...
  CentralMass_one = TestStarParticleStarMass_one*1.99e33* pow(LengthUnits*CellWidth[0][0],-3.0)/DensityUnits;
  CentralMass_two = TestStarParticleStarMass_two*1.99e33* pow(LengthUnits*CellWidth[0][0],-3.0)/DensityUnits;

  printf("Central Mass one: %f \n",CentralMass_one);
  printf("Central Mass two: %f \n",CentralMass_two);
  ...
  ParticleMass[0] = CentralMass_one;
  ...
  for (dim = 0; dim < GridRank; dim++) {
    ParticlePosition[dim][0] = TestStarParticleStarPosition_one[dim]*
      (DomainLeftEdge[dim]+DomainRightEdge[dim]) + 0.5*CellWidth[0][0];
    ParticleVelocity[dim][0] = TestStarParticleStarVelocity_one[dim]*1e5*TimeUnits/LengthUnits;

    ParticlePosition[dim][1] = TestStarParticleStarPosition_two[dim]*
      (DomainLeftEdge[dim]+DomainRightEdge[dim]) + 0.5*CellWidth[0][0];
    ParticleVelocity[dim][1] = TestStarParticleStarVelocity_two[dim]*1e5*TimeUnits/LengthUnits;
  }
  ...
  if (STARFEED_METHOD(MOM_STAR))
    if(StarMakerExplosionDelayTime >= 0.0)
      ...
      ParticleAttribute[1][1] = 10.0 * Myr_s/TimeUnits;
  
  ParticleAttribute[2][0] = 0.0;  // Metal fraction
  ParticleAttribute[3][0] = 0.0;  // metalfSNIa
  ParticleAttribute[2][1] = 0.0;  // Metal fraction
  ParticleAttribute[3][1] = 0.0;  // metalfSNIa
