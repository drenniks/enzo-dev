...
int Star::AssignFinalMassFromIMF(float TimeUnits)
{
  if (PopIII_NSM_IMF == TRUE){
    if (PopIIIInitialMassFunctionCalls == 0 || PopIIIInitialMassFunctionCalls == 1){
      ...
      float dm = log10(NSMUpperMassCutoff / NSMLowerMassCutoff) / 
        (float) (NSM_IMF_TABLE_ENTRIES-1);

      /* (binary) search for the mass bin corresponding to the random
        number */

      int width = NSM_IMF_TABLE_ENTRIES/2;
      int bin_number = NSM_IMF_TABLE_ENTRIES/2;
      
      while (width > 1) {
        width /= 2;
        if (x > NSM_IMFData[bin_number])
          bin_number += width;
        else if (x < NSM_IMFData[bin_number])
          bin_number -= width;
        else
          break;
      }
      
      this->FinalMass = NSMLowerMassCutoff * POW(10.0, bin_number * dm);
